...
namespace Server.Gumps
{
	...
	{			
		...
		public SelectTitleGump( PlayerMobile from, int page ) : base( 50, 50 )
		{			
			m_From = from;
			m_Page = page;
			...
			if ( page > -1 && page < from.CollectionTitles.Count )
			{
				if ( from.CollectionTitles[ page ] is int )
					AddHtmlLocalized( 20, 35, 230, 40, (int) from.CollectionTitles[ page ], 0x32, true, false );
				else if ( from.CollectionTitles[ page ] is string )
					AddHtml( 20, 35, 230, 40, String.Format( "<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", 0x32, (string) from.CollectionTitles[ page ] ), true, false );
			}
			else
		...
		{		
			...
			{
				case (int) Buttons.Accept: m_From.SelectCollectionTitle( m_Page ); break;
				case (int) Buttons.Next: 
					if ( m_Page == m_From.CollectionTitles.Count - 1 )
						m_From.SendGump( new SelectTitleGump( m_From, -1 ) );
					else if ( m_Page < m_From.CollectionTitles.Count - 1 && m_Page > - 1 )
