...
{
	public class AdminMenuGump : Gump
	{
		...
		public AdminMenuGump( Mobile mobile, ArrayList argsList ) : base( 0,0 )
		{
			gumpMobile = mobile;
		...
		public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
		{
			switch ( info.ButtonID )
			{
				...
				case 101: // Accounts Management Menu
				{
					...
					if ( !(bool) MenuAccessList[0] || MC.IsDefaultUser( gumpMobile ) )
					{
				...
				case 102: // Edit System Configuration
				{
					...
					if ( MC.EditSystemConfig != null )
					{
						MessagesTitle = "Edit System Configuration";
						Messages = String.Format( "{0} is currently editting the system configuration. You will have to wait until they are done.", MC.EditSystemConfig.Name );
					...
					SCGArgsList[0] = (bool) false;		// ConfigRead
					SCGArgsList[1] = TimerType.Personal;	// timerTypeSwitch
					SCGArgsList[2] = (bool) false;		// debugSwitch
					SCGArgsList[3] = (bool) false;		// staffTriggerEventSwitch

					SCGSetList[0] = (bool) false;		// TimerConfigClicked
					SCGSetList[1] = (bool) false;		// MiscConfigClicked
