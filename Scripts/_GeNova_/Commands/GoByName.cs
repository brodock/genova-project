...
{
    public class GoByNameGump : Gump
    {
        public static void Initialize()
        {
            CommandSystem.Register("GoByName", AccessLevel.GameMaster, new CommandEventHandler(GoByName_OnCommand));
        }
        public static string m_args;
        [Usage("GoByName <string>")]
        [Description("Finds all mobiles by name")]
        private static void GoByName_OnCommand(CommandEventArgs e)
        {
            m_args = e.ArgString;
            e.Mobile.SendGump(new GoByNameGump(e.Mobile, e.ArgString));
        }
        ...
        private int m_Page;
        public GoByNameGump(Mobile owner, string args)
            : this(owner, BuildList(owner, args), 0)
        {
        }

        public GoByNameGump(Mobile owner, ArrayList list, int page)
            : base(GumpOffsetX, GumpOffsetY)
        {
            owner.CloseGump(typeof(GoByNameGump));
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous 
                    {
                        if (m_Page > 0)
                            from.SendGump(new GoByNameGump(from, m_Names, m_Page - 1));
                ...
                default:
                    {
                        ...
                        if (index >= 0 && index < m_Names.Count)
                        {
                            ...
                            if (s.Deleted)
                            {
                                ...
                                from.SendGump(new GoByNameGump(from, m_Names, m_Page));
                            }
                            else
                            {
                                if (deleting)
        ...
        private class DeleteGump : Gump
        {
            ...
            public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
            {
                ...
                m_From.SendGump(new GoByNameGump(m_From, m_Names, 0));
            }
        }
