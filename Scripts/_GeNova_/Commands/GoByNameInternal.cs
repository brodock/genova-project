...
{
    public class GoByNameInternalGump : Gump
    {
        public static void Initialize()
        {
            CommandSystem.Register("GoByNameInternal", AccessLevel.GameMaster, new CommandEventHandler(GoByNameInternal_OnCommand));
        }
        public static string m_args;
        [Usage("GoByNameInternal <string>")]
        [Description("Finds all mobiles by name")]
        private static void GoByNameInternal_OnCommand(CommandEventArgs e)
        {
            m_args = e.ArgString;
            e.Mobile.SendGump(new GoByNameInternalGump(e.Mobile, e.ArgString));
        }
        ...
        private int m_Page;
        public GoByNameInternalGump(Mobile owner, string args)
            : this(owner, BuildList(owner, args), 0)
        {
        }

        public GoByNameInternalGump(Mobile owner, ArrayList list, int page)
            : base(GumpOffsetX, GumpOffsetY)
        {
            owner.CloseGump(typeof(GoByNameInternalGump));
        ...
        public static ArrayList BuildList(Mobile owner, string args)
        {
            ...
            foreach (Mobile i in World.Mobiles.Values)
            {
                ...
                {
                    if (i.Map != Map.Felucca && i.Map != Map.Trammel && i.Map != Map.Malas && i.Map != Map.Ilshenar)
                    {
                        list.Add(i);
                    }
                }
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous 
                    {
                        if (m_Page > 0)
                            from.SendGump(new GoByNameInternalGump(from, m_Names, m_Page - 1));
                ...
                default:
                    {
                        ...
                        if (index >= 0 && index < m_Names.Count)
                        {
                            ...
                            if (s.Deleted)
                            {
                                ...
                                from.SendGump(new GoByNameInternalGump(from, m_Names, m_Page));
                            }
                            else
                            {
                                if (deleting)
                                ...
                                {
                                    from.SendMessage("Going to that character.");
        ...
        private class DeleteGump : Gump
        {
            ...
            public DeleteGump(Mobile from, ArrayList spawners, int index)
                : base(50, 50)
            {
                m_From = from;
                m_Names = spawners;
                ...
                AddHtml(20, 15, 230, 60, "Are you sure you wish to delete this character?", true, true);
            ...
            public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
            {
                ...
                m_From.SendGump(new GoByNameInternalGump(m_From, m_Names, 0));
            }
        }
        ...
        private class WipeAllGump : Gump
        {
                ...
                : base(50, 50)
            {
                ...
                AddHtml(20, 15, 230, 60,
                    "Are you sure you wish to delete all of the listed character?", true, true);
            ...
            public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
            {
                if (info.ButtonID == 2)
                {
                    ...
                    m_From.SendMessage("Deleted characters.");
                }
            }
        }
    }
